...
System_Ext(payment, "Платежный сервис", "Внешний платежный сервис, через который осуществляется оплата страховок. Деньги уходят на счета страховых компаний.")
System_Ext(partners, "Системы партнеров", "Системы партнеров, интегрирующие страховые услуги в свои продукты.")
System_Ext(ins_companies, "Системы страховых компаний", "Системы страховых компаний, предоставляющие API для получения актуальных тарифов, андеррайтинга и оформления страховых продуктов.")

System_Boundary(insuretech, "InsureTech Prod\n(Виртуальная машина)") {

  Container(web, "InsureTech Web", "JavaScript, React", "Веб-приложение InsureTech")

  Container(core_app, "core-app", "Kotlin, SpringBoot",
  "Мобильное приложение InsureTech\n" +
  "- отображает данные о доступных страховых продуктах и тарифах\n" +
  ...
  ContainerDb(client_info_db, "client-info-db", "PostgreSQL",
  "БД используется для хранения данных клиентов: ФИО, телефон, паспорт и пр.\nИнформация необходима для оформления страховок")

  ContainerDb(core_db, "core-db", "PostgreSQL",
  "БД используется для хранения тарифов страховых компаний, а также заявок клиентов на страховые продукты")

  Container(product_agg, "ins-product-aggregator", "Kotlin, SpringBoot",
  "Сервис для интеграции со страховыми компаниями с целью получения информации по страховым продуктам.\nПредоставляет единый API для core")

  Container(settlement, "ins-comp-settlement", "Kotlin, SpringBoot",
  "Сервис для осуществления взаиморасчетов со страховыми компаниями")

  ContainerDb(settlement_db, "ins-comp-settlement-db", "PostgreSQL",
  "БД используется для хранения данных об оформленных страховках и условиях взаиморасчетов с клиентами")
}

' --- Relations (как на схеме) ---
...
Rel(core_app, product_agg, "Получение информации по продуктам и тарифам", "REST")
Rel(product_agg, ins_companies, "Получение информации по продуктам и тарифам", "REST / SOAP / GraphQL")

Rel(core_app, settlement, "Получение информации об оформленных страховках", "REST")
Rel(settlement, settlement_db, "Работа с данными", "TCP")
Rel(settlement, ins_companies, "Передача данных для взаиморасчетов", "REST / SOAP / GraphQL")
...
**Для схем использовать plantuml**
...
```mermaid
graph TB
    user["Браузер пользователя"] --> insuretech
    insuretech -->|TCP| postgres

    subgraph "Зона доступности 1"
        subgraph "Kubernetes cluster 1"
            insuretech["InsureTech namespace"]
        end
        subgraph "VM 1"
...
Создайте новый документ в plantuml и назовите его «InureTech_технологическая архитектура_to-be». Работайте над первым заданием там.
...
2.  Обновите [диаграмму контейнеров InsureTech](https://code.s3.yandex.net/software-architect/InsureTech_C4_%D1%81ontainer-diagram.drawio.xml), предложив решения для выявленных вами рисков и проблем. При этом:
...
7.  В зависимости от выбранных средств интеграции подумайте, требуется ли где-то применение паттернов отказоустойчивости:
    ...
    Отобразите применение паттернов [на схеме](https://code.s3.yandex.net/software-architect/%D0%9E%D0%B1%D0%BE%D0%B7%D0%BD%D0%B0%D1%87%D0%B5%D0%BD%D0%B8%D1%8F%20%D0%BF%D0%B0%D1%82%D1%82%D0%B5%D1%80%D0%BD%D0%BE%D0%BC%20%D0%BE%D1%82%D0%BA%D0%B0%D0%B7%D0%BE%D1%83%D1%81%D1%82%D0%BE%D0%B8%CC%86%D1%87%D0%B8%D0%B2%D0%BE%D1%81%D1%82%D0%B8.drawio.xml) с помощью обозначений из этой библиотеки.
